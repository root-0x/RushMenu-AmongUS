...
public static class MalumSpoof
{
    ...
    public static void spoofLevel()
    {
        ...
        if (!string.IsNullOrEmpty(RushMenu.spoofLevel.Value) && 
            uint.TryParse(RushMenu.spoofLevel.Value, out parsedLevel) &&
    ...
    {
        string friendCode = RushMenu.guestFriendCode.Value;
    ...
    public static void spoofPlatform(PlatformSpecificData platformSpecificData)
    {
        ...
        if (Utils.stringToPlatformType(RushMenu.spoofPlatform.Value, out platformType))
        {
