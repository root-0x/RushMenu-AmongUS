...
public static class SystemInfo_deviceUniqueIdentifier_Getter
{
    ...
    {
        if (!RushMenu.spoofDeviceId.Value) return;
...
public static class AmongUsClient_Update
{
    public static void Postfix()
    {
        ...
        if (!EOSManager.Instance.loginFlowFinished || !RushMenu.guestMode.Value) return;
...
{
    // Postfix patch of VersionShower.Start to show RushMenu version
    public static void Postfix(VersionShower __instance)
    {
        const string displayDate = "2026.02.12";
        if (RushMenu.supportedAU.Contains(Application.version)){ // Checks if Among Us version is supported

            __instance.text.text =  $"RushMenu v{RushMenu.malumVersion} (v{displayDate})"; // Supported

        }else{

            __instance.text.text =  $"RushMenu v{RushMenu.malumVersion} (<color=red>v{displayDate}</color>)"; //Unsupported
...
public static class PingTracker_Update
{
    ...
    public static void Postfix(PingTracker __instance)
    {
        ...
        if (AmongUsClient.Instance.IsGameStarted){
            ...
            __instance.text.text = $"RushMenu By Slayy ~ {Utils.getColoredPingText(AmongUsClient.Instance.Ping)}";

            return;
        }

        __instance.text.text = $"RushMenu By Slayy\n{Utils.getColoredPingText(AmongUsClient.Instance.Ping)}";
