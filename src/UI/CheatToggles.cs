...
public struct CheatToggles
{
    ...
    /// <summary>
    /// Saves all cheat toggles to a file named "RushProfile.txt" in the BepInEx config directory.
    ...
    {
        var fields = typeof(CheatToggles).GetFields(System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
        using var writer = new StreamWriter(Path.Combine(BepInEx.Paths.ConfigPath, "RushProfile.txt"));
        foreach (var field in fields)
        {
            if (field.FieldType == typeof(bool))
            {
    ...
    /// <summary>
    /// Loads cheat toggles from a file named "RushProfile.txt" in the BepInEx config directory.
    /// Each line in the file contains a toggle name and its value in the format "ToggleName=true/false".
    /// </summary>
    public static void LoadAllFromFile()
    {
        ...
        var filePath = Path.Combine(BepInEx.Paths.ConfigPath, "RushProfile.txt");
